...
export function DashboardPage() {
  ...
  const indicators = [
    ...
    {
      detail: 'Autorizações previstas no dia',
      ...
      <header className="flex flex-col gap-6 border-b border-ink/10 pb-7 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-3xl">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-ink/65 sm:text-base">
            Acompanhe a movimentação do campus e encontre rapidamente o que precisa para o turno de hoje.
          </p>
        </div>
      ...
      >
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-ink/8 px-5 py-4 sm:px-7">
          <div>
            ...
            <h2 className="mt-1 font-display text-2xl text-ink" id="daily-summary-title">
              Resumo de hoje
            </h2>
          </div>
        ...
        <div className="grid gap-px bg-ink/8 sm:grid-cols-2 xl:grid-cols-4">
          {indicators.map((indicator) => (
            <article className={`${indicator.surface} min-h-36 p-5 sm:p-6`} key={indicator.label}>
              <div className="flex items-start justify-between gap-4">
                <p className="max-w-32 text-sm font-bold leading-5 text-ink/75">{indicator.label}</p>
                <strong className="font-display text-4xl font-normal leading-none text-ink">
                  {indicator.value.toString().padStart(2, '0')}
                </strong>
              </div>
              <p className="mt-6 text-xs leading-5 text-ink/60">{indicator.detail}</p>
            </article>
          ))}
        </div>
      </section>
      ...
      <div className="mt-7 grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1.45fr)_minmax(19rem,0.55fr)]">
        ...
        >
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.15em] text-brand-dark">Portaria</p>
              <h2 className="mt-1 font-display text-3xl text-ink" id="recent-movements-title">
                Movimentações recentes
              </h2>
              <p className="mt-2 text-sm text-ink/55">Últimos registros realizados no campus.</p>
            </div>
            <Link
          ...
          <div className="mt-6 space-y-3 sm:hidden">
            {records.slice(0, 4).map((record) => (
              <article className="rounded-2xl border border-ink/10 bg-cream/35 p-4" key={record.id}>
                ...
                <p className="mt-3 border-t border-ink/8 pt-3 text-xs leading-5 text-ink/65">
                  {record.destination} • {dateTimeFormatter.format(new Date(record.entryAt))}
                </p>
              </article>
            ))}
          </div>
          ...
          <div className="mt-6 hidden max-w-full overflow-x-auto sm:block">
            <table className="w-full min-w-[40rem] border-collapse text-left text-sm">
              <caption className="sr-only">Movimentações recentes do campus</caption>
              <thead>
                <tr className="border-b border-ink/10 text-[0.68rem] uppercase tracking-[0.12em] text-ink/50">
                  <th className="px-3 py-3 font-bold" scope="col">Veículo</th>
                  <th className="px-3 py-3 font-bold" scope="col">Condutor</th>
                  <th className="px-3 py-3 font-bold" scope="col">Destino</th>
                  <th className="px-3 py-3 font-bold" scope="col">Entrada</th>
                  <th className="px-3 py-3 font-bold" scope="col">Situação</th>
                </tr>
              </thead>
              <tbody>
                {records.slice(0, 4).map((record) => (
                  <tr className="border-b border-ink/6 last:border-0" key={record.id}>
                    ...
                    <td className="px-3 py-4 text-ink/70">{record.destination}</td>
        ...
        <aside className="space-y-6">
          <section className="rounded-[2rem] bg-[#B8C9A4] p-5 sm:p-6" aria-labelledby="quick-actions-title">
            <p className="text-xs font-bold uppercase tracking-[0.15em] text-ink/60">Atalhos</p>
            <h2 className="mt-1 font-display text-2xl text-ink" id="quick-actions-title">
              ...
              {quickActions.map((action) => (
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="mt-0.5 block text-xs leading-4 text-ink/55">{action.description}</span>
                  </span>
                  <Icon
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.14em] text-ink/55">Turno atual</p>
                <h2 className="font-display text-xl text-ink" id="shift-notes-title">Pontos de atenção</h2>
              </div>
            </div>
            <ul className="mt-5 space-y-3 text-sm leading-5 text-ink/70">
              <li className="flex gap-3">
                <span aria-hidden="true" className="mt-1.5 size-2 shrink-0 rounded-full bg-[#c90f11]" />
              ...
              <li className="flex gap-3">
                ...
                1 veículo institucional aparece em movimentação.
              </li>
              <li className="flex gap-3">
                <span aria-hidden="true" className="mt-1.5 size-2 shrink-0 rounded-full bg-brand" />
